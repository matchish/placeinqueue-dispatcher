...
describe("spread", function () {
    ...
    it("if range is 1-100000 it produce 100 events to dispatcher queue", async function () {
        ...
        let events = await new SpreadEvents(body, dao, moment, {
    ...
    it("if range is 95-110 and all browsers stopped it produce 16 events to browser queue", async function () {
        //TODO mock now
        let heartbeatAt = moment().subtract(90, "seconds").toISOString();
        let dao = {
            PlaceDao: class {
                readEntity(key) {
    ...
    it("if doesn't produce events to browser if place has number", async function () {
        //TODO mock now
